...
/*    public void calcularTotal(Venta venta, int prueba) throws ExcepcionPersonalizada {

        if (productos == null) {
            throw new ExcepcionPersonalizada("Para hacer la venta, debes añadir productos");
        }
        ArrayList <Producto> producto= (ArrayList<Producto>) venta.getProductos();
        ...
        for (int i = 0; i < size; i++) {
            ...
            venta.precioTotal = venta.getPrecioTotal()+ producto.get(i).getPrecio();
        }
    ...
    public void calcularTotal( int prueba) throws ExcepcionPersonalizada {
        ...
        //ArrayList <Producto> producto= (ArrayList<Producto>) productos.getProductos();
        int size=productos.size();
        for (int i = 0; i < size; i++) {
            if(prueba==1){
                i=3;
